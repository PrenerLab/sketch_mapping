...
``` r
cluster1 <- qm_define("F-2", "F-3", "G-2", "G-3")
```
...
``` r
cluster2 <- qm_define("C-2", "C-3", "D-2", "D-3")
cluster3 <- qm_define("D-2")
cluster4 <- qm_define("E-4")
```
...
cluster2 <- qm_create(ref = grids, key = GRID_ID, value = cluster2, rid = params$pid, cid = 2, category = "regular")
...
``` r
clusters <- qm_combine(cluster1, cluster2, cluster3, cluster4)
```
