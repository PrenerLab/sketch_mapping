...
labels <- tibble(
  ...
  x = c(-90.227857, -90.231888),
  y = c(38.637693, 38.620462)
)
...
features <- tibble(
  ...
  x = c(-90.274276, -90.274036, -90.274161),
  y = c(38.617122, 38.628091, 38.641927)
)
...
tm_shape(freq_clusters) +
  ...
  tm_shape(labels) +
    tm_text("name", fontface = 2, just = "left", size = .85, shadow = TRUE) +
  tm_shape(nhood) +
  ...
  tm_credits("Map by Christopher Prener, Ph.D.\nCount of clusters for visiting areas at least twice per week", 
             position=c("left", "bottom"), just = "bottom") +
  tm_legend(outside = TRUE, attr.outside = TRUE) -> freq_clusters_map

tmap_save(tm = freq_clusters_map, filename = "freq_clusters_map.png", dpi = 500)
```
...
tm_shape(freq_respondents) +
  ...
  tm_layout(title = "Frequent Visits \nby Respondent",
            frame = FALSE, outer.margins = c(0,0,0,0)) +
  tm_credits("Map by Christopher Prener, Ph.D.\n% of respondents who visit areas at least twice per week", 
             position=c("left", "bottom"), just = "bottom") +
  tm_legend(outside = TRUE, attr.outside = TRUE) -> freq_respondents_map

tmap_save(tm = freq_respondents_map, filename = "freq_respondents_map.png", dpi = 500)
```
...
tm_shape(reg_clusters) +
  ...
  tm_layout(title = "Regular Visits \nby Cluster",
            frame = FALSE, outer.margins = c(0,0,0,0)) +
  tm_credits("Map by Christopher Prener, Ph.D.\nCount of clusters for visiting areas two to four times per month", 
             position=c("left", "bottom"), just = "bottom") +
  tm_legend(outside = TRUE, attr.outside = TRUE) -> reg_clusters_map

tmap_save(tm = reg_clusters_map, filename = "reg_clusters_map.png", dpi = 500)
```
...
tm_shape(reg_respondents) +
  ...
  tm_layout(title = "Regular Visits \nby % of Respondents",
            frame = FALSE, outer.margins = c(0,0,0,0)) +
  tm_credits("Map by Christopher Prener, Ph.D.\n% of respondents who visit areas two to four times per month", 
             position=c("left", "bottom"), just = "bottom") +
  tm_legend(outside = TRUE, attr.outside = TRUE) -> reg_respondents_map

tmap_save(tm = reg_respondents_map, filename = "reg_respondents_map.png", dpi = 500)
```
